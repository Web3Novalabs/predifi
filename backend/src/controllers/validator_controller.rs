...
fn validate_contract_address(address: &str) -> Result<(), AppError> {
    let is_valid = address.starts_with("0x") && address.len() == 42 && address.chars().all(|c| c.is_ascii_hexdigit() || c == 'x');
    if !is_valid {
        Err(AppError::BadRequest("Invalid contract address format".into()))
    } else {
        Ok(())
    }
}
