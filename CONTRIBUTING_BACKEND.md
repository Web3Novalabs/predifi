...
Run the integration migration round-trip test which spins up a throwaway
Postgres container, applies the migrations and verifies they can be reverted:

```bash
cd backend
cargo test --test migration_roundtrip
```
