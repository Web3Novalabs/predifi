...
fn test_init_oracle_rejects_zero_max_price_age() {
    let env = Env::default();
    env.mock_all_auths();
    let (ac_client, client, _token_address, _, _, _, _operator, _) = setup(&env);
    let admin = Address::generate(&env);
    ac_client.grant_role(&admin, &ROLE_ADMIN);
...
/// Shared helper: create a minimal 2-outcome pool ending at timestamp 100_000.
fn create_basic_pool(
    env: &Env,
    client: &PredifiContractClient<'_>,
    token_address: &Address,
    creator: &Address,
) -> u64 {
    client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(env, "Staking close test pool"),
            metadata_url: String::from_str(
                env,
                "ipfs://bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi",
            ),
...
#[test]
fn test_close_staking_emits_event() {
    let env = Env::default();
    env.mock_all_auths();
    env.ledger().with_mut(|li| li.timestamp = 1_000);

    let (_, client, token_address, _, _, _, _, creator) = setup(&env);
    let pool_id = create_basic_pool(&env, &client, &token_address, &creator);
    ...
    // Verify that exactly one "staking_closed" event was emitted and check payload.
    let events = env.events().all();
    let staking_closed_topic = Symbol::new(&env, "staking_closed");
    let mut found = false;
    ...
    for e in events.iter() {
        if let Some(topic_val) = e.1.get(0) {
            if let Ok(topic_sym) = Symbol::try_from_val(&env, &topic_val) {
                if topic_sym == staking_closed_topic {
                    match_count += 1;
                    if !found {
                        let data: soroban_sdk::Map<Symbol, Val> = e.2.clone().into_val(&env);

                        let emitted_pool_id: u64 = data
                            .get(Symbol::new(&env, "pool_id"))
                            .unwrap()
                            .into_val(&env);
                        let emitted_end_time: u64 = data
                            .get(Symbol::new(&env, "end_time"))
                            .unwrap()
                            .into_val(&env);
                        let emitted_timestamp: u64 = data
                            .get(Symbol::new(&env, "timestamp"))
                            .unwrap()
                            .into_val(&env);

                        assert_eq!(emitted_pool_id, pool_id);
                        assert_eq!(emitted_end_time, 100_000u64);
                        assert_eq!(emitted_timestamp, 100_001u64);
                        found = true;
                    }
                }
            }
    ...
    assert!(found, "StakingClosedEvent not found");
    assert_eq!(match_count, 1, "Expected exactly one StakingClosedEvent");
}

#[test]
fn test_close_staking_idempotent_no_duplicate_event() {
    let env = Env::default();
    env.mock_all_auths();
    env.ledger().with_mut(|li| li.timestamp = 1_000);
    ...
    // Helper: count staking_closed events in the current event log snapshot.
    let count_staking_closed = |topic: &Symbol| -> u32 {
        let mut n = 0u32;
        for e in env.events().all().iter() {
            if let Some(tv) = e.1.get(0) {
                if let Ok(s) = Symbol::try_from_val(&env, &tv) {
                    if &s == topic {
                        n += 1;
                    }
                }
    ...
    // First call — event should be emitted once.
    client.close_staking(&pool_id);
    assert_eq!(
        count_staking_closed(&staking_closed_topic),
        1,
        "Expected one StakingClosedEvent after first call"
    );

    // Second call — must succeed but emit NO new event.
    // The Soroban test env clears the log between top-level calls, so after the
    // second call the log reflects only what that call emitted (nothing).
    client.close_staking(&pool_id);
    assert_eq!(
        count_staking_closed(&staking_closed_topic),
        0,
        "StakingClosedEvent must NOT be re-emitted on a second call (idempotency check)"
    );
}

#[test]
fn test_close_staking_fails_before_end_time() {
    let env = Env::default();
    env.mock_all_auths();
    env.ledger().with_mut(|li| li.timestamp = 1_000);
    ...
    // Still before end_time; staking window is open.
    let result = client.try_close_staking(&pool_id);
    assert!(
        result.is_err(),
        "close_staking should fail when end_time has not been reached"
    );
}

#[test]
fn test_close_staking_fails_for_resolved_pool() {
    let env = Env::default();
    env.mock_all_auths();
    env.ledger().with_mut(|li| li.timestamp = 1_000);
    ...
    // Resolve the pool before calling close_staking.
    env.ledger().with_mut(|li| li.timestamp = 100_001);
    client.resolve_pool(&operator, &pool_id, &0u32);

    // A resolved pool is not Active; close_staking should return InvalidPoolState.
    let result = client.try_close_staking(&pool_id);
    assert!(
        result.is_err(),
...
// ═══════════════════════════════════════════════════════════════════════════
// Issue #1125 — Storage TTL Renewal Helper Tests
// ═══════════════════════════════════════════════════════════════════════════

/// renew_storage_ttl succeeds for an existing pool and emits the event.
#[test]
fn test_renew_storage_ttl_succeeds_for_existing_pool() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "TTL renewal test"),
            metadata_url: String::from_str(&env, "ipfs://ttl"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    client.renew_storage_ttl(&pool_id);

    // Verify the StorageTtlRenewedEvent was emitted.
    let events = env.events().all();
    let topic = Symbol::new(&env, "storage_ttl_renewed");
    let found = events.iter().any(|e| {
        e.1.get(0)
            .and_then(|v| Symbol::try_from_val(&env, &v).ok())
            .map(|s| s == topic)
    ...
    assert!(found, "StorageTtlRenewedEvent should be emitted");
}

/// renew_storage_ttl returns PoolNotFound for a non-existent pool.
#[test]
fn test_renew_storage_ttl_fails_for_unknown_pool() {
    let env = Env::default();
    env.mock_all_auths();

    let (_, client, _, _, _, _, _, _) = setup(&env);

    let result = client.try_renew_storage_ttl(&9999u64);
    assert_eq!(
        result,
        Err(Ok(PredifiError::PoolNotFound)),
        "should return PoolNotFound for non-existent pool"
    );
}

/// renew_storage_ttl is callable by anyone (permissionless).
#[test]
fn test_renew_storage_ttl_is_permissionless() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Permissionless TTL"),
            metadata_url: String::from_str(&env, "ipfs://perm"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    // Any random address can trigger the renewal (no role required).
    client.renew_storage_ttl(&pool_id);
}

// ═══════════════════════════════════════════════════════════════════════════
// Issue #1128 — Referral Volume Threshold Tests
// ═══════════════════════════════════════════════════════════════════════════

/// Default threshold is 0 (no minimum — all referrers qualify).
#[test]
fn test_referral_volume_threshold_default_is_zero() {
    let env = Env::default();
    env.mock_all_auths();

    let (_, client, _, _, _, _, _, _) = setup(&env);

    assert_eq!(client.get_referral_volume_threshold(), 0i128);
}

/// Admin can set and read back a positive threshold.
#[test]
fn test_set_referral_volume_threshold_admin_success() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.set_referral_volume_threshold(&admin, &500i128);
    assert_eq!(client.get_referral_volume_threshold(), 500i128);
}

/// Non-admin cannot set the threshold.
#[test]
#[should_panic(expected = "Error(Contract, #10)")]
fn test_set_referral_volume_threshold_unauthorized() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.set_referral_volume_threshold(&not_admin, &100i128);
}

/// Emits ReferralThresholdUpdatedEvent.
#[test]
fn test_set_referral_volume_threshold_emits_event() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.set_referral_volume_threshold(&admin, &200i128);

    let topic = Symbol::new(&env, "referral_threshold_updated");
    ...
    assert!(found, "ReferralThresholdUpdatedEvent should be emitted");
}

/// Referrer below threshold does NOT receive referral reward.
#[test]
fn test_referral_volume_threshold_blocks_low_volume_referrer() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Set threshold to 1000 base units — referred volume (100) will be below it.
    client.set_referral_volume_threshold(&admin, &1000i128);
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Threshold block test"),
            metadata_url: String::from_str(&env, "ipfs://threshold"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    // User stakes 100 — referred volume (100) is below threshold (1000).
    client.place_prediction(
        &user,
        &pool_id,
        &100,
    ...
    // Referrer should receive nothing because volume is below threshold.
    assert_eq!(token.balance(&referrer), 0i128, "referrer below threshold should get 0");
}

/// Referrer at or above threshold DOES receive the referral reward.
#[test]
fn test_referral_volume_threshold_allows_qualifying_referrer() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Set threshold to 50 — referred volume (100) is above it.
    client.set_referral_volume_threshold(&admin, &50i128);
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Threshold pass test"),
            metadata_url: String::from_str(&env, "ipfs://pass"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    // User stakes 100 — referred volume (100) meets threshold (50).
    client.place_prediction(
        &user,
        &pool_id,
        &100,
    ...
    // Referrer should receive the referral reward (50% of 2% fee = 1 token).
    assert!(token.balance(&referrer) > 0i128, "referrer above threshold should get a reward");
}

// ═══════════════════════════════════════════════════════════════════════════
// Issue #1137 — Contract Metadata Getter Tests
// ═══════════════════════════════════════════════════════════════════════════

/// get_contract_metadata returns a fully populated struct.
#[test]
fn test_get_contract_metadata_returns_populated_struct() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Create a pool so total_pools > 0.
    client.create_pool(
        &creator,
        &100_000u64,
        &token_address,
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Metadata test"),
            metadata_url: String::from_str(&env, "ipfs://meta"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
    ...
    assert_eq!(metadata.version_string, Symbol::new(&env, "0_0_0"));
    assert!(!metadata.is_paused);
    assert_eq!(metadata.total_pools, 1u64);
    assert_eq!(metadata.active_pools_count, 1u32);
    assert_eq!(metadata.fee_bps, 0u32);           // setup initializes with 0
    assert_eq!(metadata.referral_cut_bps, 5000u32); // default
    assert_eq!(metadata.referral_min_volume, 0i128); // no threshold set
    assert!(!metadata.oracle_initialized);          // oracle not init'd in setup
    assert_eq!(metadata.fee_tiers_count, 0u32);
}

/// get_contract_metadata reflects admin configuration changes.
#[test]
fn test_get_contract_metadata_reflects_config_changes() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    assert_eq!(metadata.fee_bps, 300u32);
    assert_eq!(metadata.referral_cut_bps, 2000u32);
    assert_eq!(metadata.referral_min_volume, 500i128);
    assert!(metadata.is_paused);
}

/// get_contract_metadata matches get_contract_info for shared fields.
#[test]
fn test_get_contract_metadata_consistent_with_get_contract_info() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    assert_eq!(info.version, meta.version);
    assert_eq!(info.is_paused, meta.is_paused);
    assert_eq!(info.total_pools, meta.total_pools);
    assert_eq!(info.fee_bps, meta.fee_bps);
    assert_eq!(info.referral_cut_bps, meta.referral_cut_bps);
    assert_eq!(info.treasury, meta.treasury);
    assert_eq!(info.access_control, meta.access_control);
    assert_eq!(info.resolution_delay, meta.resolution_delay);
    assert_eq!(info.min_pool_duration, meta.min_pool_duration);
    assert_eq!(info.min_stake, meta.min_stake);
    assert_eq!(info.max_predictions_per_user, meta.max_predictions_per_user);
    assert_eq!(info.prediction_cooldown_seconds, meta.prediction_cooldown_seconds);
}

/// active_pools_count in metadata decreases when a pool is resolved.
#[test]
fn test_get_contract_metadata_active_pools_count_decreases_on_resolve() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Active count test"),
            metadata_url: String::from_str(&env, "ipfs://count"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    assert_eq!(client.get_contract_metadata().active_pools_count, 1u32);

    env.ledger().with_mut(|li| li.timestamp = 100_001);
    client.resolve_pool(&operator, &0u64, &0u32);

    assert_eq!(client.get_contract_metadata().active_pools_count, 0u32);
}

// ═══════════════════════════════════════════════════════════════════════════
// Issue #1142 — Event Emission Consistency Tests
// ═══════════════════════════════════════════════════════════════════════════

/// update_referrer now emits ReferrerUpdatedEvent when setting a referrer.
#[test]
fn test_update_referrer_emits_event_on_set() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Referrer event test"),
            metadata_url: String::from_str(&env, "ipfs://ref"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    client.update_referrer(&user, &pool_id, &Some(referrer.clone()));

    let topic = Symbol::new(&env, "referrer_updated");
    let found = env.events().all().iter().any(|e| {
        e.1.get(0)
            .and_then(|v| Symbol::try_from_val(&env, &v).ok())
            .map(|s| s == topic)
    ...
    assert!(found, "ReferrerUpdatedEvent should be emitted when referrer is set");
}

/// update_referrer emits ReferrerUpdatedEvent when clearing (None).
#[test]
fn test_update_referrer_emits_event_on_clear() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Clear referrer test"),
            metadata_url: String::from_str(&env, "ipfs://clear"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    // Set then clear.
    client.update_referrer(&user, &pool_id, &Some(referrer));
    client.update_referrer(&user, &pool_id, &None);

    let topic = Symbol::new(&env, "referrer_updated");
    let count = env.events().all().iter().filter(|e| {
        e.1.get(0)
            .and_then(|v| Symbol::try_from_val(&env, &v).ok())
            .map(|s| s == topic)
            .unwrap_or(false)
    ...
    assert_eq!(count, 2, "ReferrerUpdatedEvent should be emitted on each call");
}

/// increase_max_total_stake emits MaxTotalStakeIncreasedEvent.
#[test]
fn test_increase_max_total_stake_emits_event() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Max stake event test"),
            metadata_url: String::from_str(&env, "ipfs://maxstake"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 100i128,
            min_total_stake: 1,
    ...
    client.increase_max_total_stake(&creator, &pool_id, &500i128);

    let topic = Symbol::new(&env, "max_stake_increased");
    let found = env.events().all().iter().any(|e| {
        e.1.get(0)
            .and_then(|v| Symbol::try_from_val(&env, &v).ok())
            .map(|s| s == topic)
    ...
    assert!(found, "MaxTotalStakeIncreasedEvent should be emitted");
}

/// set_price_condition emits PriceConditionSetEvent.
#[test]
fn test_set_price_condition_emits_event() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &PoolConfig {
            start_time: 0,
            description: String::from_str(&env, "Price condition event test"),
            metadata_url: String::from_str(&env, "ipfs://price"),
            min_stake: 1i128,
            max_stake: 0i128,
            max_total_stake: 0,
            min_total_stake: 1,
    ...
    let topic = Symbol::new(&env, "price_condition_set");
    let found = env.events().all().iter().any(|e| {
        e.1.get(0)
            .and_then(|v| Symbol::try_from_val(&env, &v).ok())
            .map(|s| s == topic)
    ...
    assert!(found, "PriceConditionSetEvent should be emitted by set_price_condition");
}
