...
/// Placing a prediction without a referrer results in zero referral payout.
#[test]
fn test_no_referrer_no_payout() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let token = token::Client::new(&env, &token_contract);
    let token_admin_client = token::StellarAssetClient::new(&env, &token_contract);
    let token_address = token_contract.clone();
    ...
    ac_client.grant_role(&admin, &ROLE_ADMIN);
    ac_client.grant_role(&operator, &ROLE_OPERATOR);
    client.init(&ac_id, &treasury, &200u32, &0u64);
    client.add_token_to_whitelist(&admin, &token_address);
    ...
    let pool_id = client.create_pool(
        ...
        &symbol_short!("Tech"),
        &PoolConfig {
            description: String::from_str(&env, "No referrer pool"),
            metadata_url: String::from_str(&env, "ipfs://test"),
            min_stake: 1i128,
            max_stake: 0i128,
            initial_liquidity: 0i128,
    ...
    let winnings = client.claim_winnings(&user, &pool_id);
    // Protocol fee = 2% of 100 = 2. Payout = 98.
    assert_eq!(winnings, 98);
    // No referrer: contract retains the 2 fee tokens (treasury hasn't withdrawn yet).
    assert_eq!(token.balance(&client.address), 2);
}

/// Referred volume accumulates correctly across multiple predictions by the same user.
#[test]
fn test_referred_volume_accumulates() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &symbol_short!("Tech"),
        &PoolConfig {
            description: String::from_str(&env, "Volume accumulation pool"),
            metadata_url: String::from_str(&env, "ipfs://test"),
            min_stake: 1i128,
            max_stake: 0i128,
            initial_liquidity: 0i128,
    ...
    let referrer = Address::generate(&env);
    let referred_user = Address::generate(&env);
    token_admin_client.mint(&referred_user, &10_000);

    // First prediction sets referrer and records 200 volume.
    client.place_prediction(
        &referred_user,
        &pool_id,
        &200,
    ...
    assert_eq!(client.get_referred_volume(&referrer, &pool_id), 200);

    // Second prediction: referrer arg is ignored (already locked), but volume still accumulates.
    client.place_prediction(&referred_user, &pool_id, &300, &0, &None, &None);
    assert_eq!(client.get_referred_volume(&referrer, &pool_id), 500);
}

/// The referrer set on the first prediction cannot be overridden by a different referrer
/// on a subsequent prediction by the same user in the same pool.
#[test]
fn test_referrer_locked_after_first_prediction() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &symbol_short!("Tech"),
        &PoolConfig {
            description: String::from_str(&env, "Locked referrer pool"),
            metadata_url: String::from_str(&env, "ipfs://test"),
            min_stake: 1i128,
            max_stake: 0i128,
            initial_liquidity: 0i128,
    ...
    // First prediction sets referrer.
    client.place_prediction(
        &referred_user,
        &pool_id,
        &100,
    ...
    // Second prediction tries to set a different referrer — should be ignored.
    client.place_prediction(
        &referred_user,
        &pool_id,
        &100,
    ...
    env.ledger().with_mut(|li| li.timestamp = 100001);
    client.resolve_pool(&operator, &pool_id, &0u32);
    client.claim_winnings(&referred_user, &pool_id);

    // Only the original referrer should have received a payout.
    assert!(
        token.balance(&referrer) > 0,
        "original referrer should be paid"
    );
    assert_eq!(
        token.balance(&other_referrer),
        0,
        "second referrer should not be paid"
    );
}

/// Referral payout is zero when protocol fee_bps is 0.
#[test]
fn test_referral_zero_fee_bps_no_payout() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &symbol_short!("Tech"),
        &PoolConfig {
            description: String::from_str(&env, "Zero fee pool"),
            metadata_url: String::from_str(&env, "ipfs://test"),
            min_stake: 1i128,
            max_stake: 0i128,
            initial_liquidity: 0i128,
    ...
    // No protocol fee means no referral cut.
    assert_eq!(winnings, 100);
    assert_eq!(token.balance(&referrer), 0);
}

/// Referrer cannot be the same address as the user placing the prediction.
#[test]
#[should_panic]
fn test_referrer_cannot_be_self() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pool_id = client.create_pool(
        ...
        &symbol_short!("Tech"),
        &PoolConfig {
            description: String::from_str(&env, "Self-ref pool"),
            metadata_url: String::from_str(&env, "ipfs://test"),
            min_stake: 1i128,
            max_stake: 0i128,
            initial_liquidity: 0i128,
    ...
    // Should panic: user refers themselves.
    client.place_prediction(&user, &pool_id, &100, &0, &Some(user.clone()), &None);
}

/// get_referred_volume returns 0 for an address that has never been a referrer.
#[test]
fn test_get_referred_volume_unknown_referrer() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    assert_eq!(client.get_referred_volume(&unknown, &42u64), 0);
}

/// Two different users referred by the same referrer in the same pool both
/// contribute to the referrer's volume and both trigger a referral payout on claim.
#[test]
fn test_multiple_users_same_referrer() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    ac_client.grant_role(&admin, &ROLE_ADMIN);
    ac_client.grant_role(&operator, &ROLE_OPERATOR);
    client.init(&ac_id, &treasury, &200u32, &0u64); // 2% fee
    client.add_token_to_whitelist(&admin, &token_address);
    client.set_referral_cut_bps(&admin, &5000u32); // 50% referral cut
    ...
    let pool_id = client.create_pool(
        ...
        &symbol_short!("Tech"),
        &PoolConfig {
            description: String::from_str(&env, "Multi-user referral pool"),
            metadata_url: String::from_str(&env, "ipfs://test"),
            min_stake: 1i128,
            max_stake: 0i128,
            initial_liquidity: 0i128,
    ...
    let referrer = Address::generate(&env);
    let referred_user1 = Address::generate(&env);
    let referred_user2 = Address::generate(&env);
    token_admin_client.mint(&referred_user1, &10_000);
    token_admin_client.mint(&referred_user2, &10_000);

    // Both users bet on outcome 0 with the same referrer.
    client.place_prediction(
        &referred_user1,
        &pool_id,
        &100,
    ...
    client.claim_winnings(&referred_user1, &pool_id);
    client.claim_winnings(&referred_user2, &pool_id);

    // Each user: 2% fee on 100 = 2, referrer gets 50% = 1 per user → 2 total.
