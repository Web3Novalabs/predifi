...
fn test_get_user_predictions() {
    ...
    let pool0 = client.create_pool(&creator, &100, &token_address, &category, &options);
    let pool1 = client.create_pool(&creator, &200, &token_address, &category, &options);
    let pool2 = client.create_pool(&creator, &300, &token_address, &category, &options);
...
fn test_create_pool_validation() {
    ...
    // Past end_time
    let result = client.try_create_pool(&creator, &50, &token_address, &category, &2);
    assert_eq!(result, Err(Ok(PrediFiError::EndTimeMustBeFuture)));

    // Invalid options count
    let result = client.try_create_pool(&creator, &200, &token_address, &category, &1);
    assert_eq!(result, Err(Ok(PrediFiError::InvalidOptionsCount)));

    // Valid
    let result = client.try_create_pool(&creator, &200, &token_address, &category, &2);
    assert!(result.is_ok());
}
...
fn test_unauthorized_pool_creation() {
    ...
    // This should fail because we aren't providing the creator's signature
    client.create_pool(&creator, &200, &token, &category, &options);
}

#[test]
fn test_fee_precision_small_amounts() {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register(PredifiContract, ());
    let client = PredifiContractClient::new(&env, &contract_id);
    ...
    let user = Address::generate(&env);
    token_admin_client.mint(&user, &10); // Very small amount

    client.init(&Address::generate(&env), &100); // 1%
    ...
    let pool_id = client.create_pool(&user, &100, &token_contract, &Symbol::new(&env, "test"), &2);

    client.place_prediction(&user, &user, &pool_id, &10, &1);
    env.ledger().set_timestamp(101);
    client.resolve_pool(&user, &pool_id, &1);

    // 1% of 10 is 0.1. In integer math, this will likely be 0.
    // We need to ensure the contract handles 0 fee gracefully.
