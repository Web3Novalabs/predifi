...
pub enum DataKey {
    ...
    /// Latest price feed data: `PriceFeed(feed_pair)` -> `PriceFeed`
    ///
    /// Used by both `price_feed.rs` (full adapter) and `price_feed_simple.rs`.
    PriceFeed(Symbol),
    /// Price condition for automated resolution: `PriceCondition(pool_id)` -> `PriceCondition`
    ///
    /// Used by both `price_feed.rs` (full adapter) and `price_feed_simple.rs`.
    PriceCondition(u64),
    /// Global oracle configuration: `OracleConfig` -> `OracleConfig`
    ///
    /// Stores Pyth contract address and validation parameters.
    /// Previously stored ad-hoc via `TokenWl(pyth_contract)` in `price_feed_simple.rs`.
    OracleConfig,
}
